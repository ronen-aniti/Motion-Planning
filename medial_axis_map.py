...
class MedialAxisGridMap(Map):
    ...
    def search_grid(self):
        ...
        current_north, current_east = self.find_closest(self.current_grid_location)
        goal_north, goal_east = self.find_closest(self.goal_grid_location)
    ...
    def find_closest(self, gridcell):
        ...
        """
        freegrid_cells = np.transpose(np.nonzero(self.grid))
        nearest_start_idx = np.linalg.norm(np.array(gridcell) - np.array(self.grid)).argmin()


        """
        gridcell_north, gridcell_east = gridcell
        d = 1000000
        closest_cell = None
        for i in range(len(self.grid)):
            for j in range(len(self.grid[0])):
                if self.grid[i,j] == False:
                    computed_distance = np.sqrt((i - gridcell_north)**2 + (j - gridcell_east)**2)
                    if computed_distance < d:
                        d = computed_distance
                        closest_cell = (i, j)
        print('closest gridcell to destination is', closest_cell)
        return closest_cell
        """
